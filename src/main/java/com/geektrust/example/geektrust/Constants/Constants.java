package com.geektrust.example.geektrust.Constants;

public class Constants {
    public static final String ENGINE = "ENGINE";
    public static final String ARRIVAL = "ARRIVAL";
    public static final String DEPARTURE = "DEPARTURE";
    public static final String JOURNEY_ENDED = "JOURNEY_ENDED";
    public static final String LOAD_DATA = "LOAD_DATA";
    public static final String space = " ";
    public static final String TRAVEL = "TRAVEL";
    public static final String FIRST_TRAIN = "TRAIN_A";
    public static final String SECOND_TRAIN = "TRAIN_B";
    public static final String HYB = "HYB";
    public static final String MERGE  = "MERGE";
    public static final String MERGED_TRAIN = "TRAIN_AB";
    public static final boolean FALSE = false;
    public static final int ZERO = 0;
    public static final String RANDOM = "qwertyuiopasdfghjklzxcvbnm";
    public static final String Chennai = "Chennai";
    public static final String Salem = "Salem";
    public static final String Bangalore = "Bangalore";
    public static final String Kurnool = "Kurnool";
    public static final String Hyderabad = "Hyderabad";
    public static final String Nagpur = "Nagpur";
    public static final String Itarsi = "Itarsi";
    public static final String Bhopal = "Bhopal";
    public static final String Agra = "Agra";
    public static final String New_Delhi = "New Delhi";
    public static final String Trivandrum = "Trivandrum";
    public static final String Shoranpur = "Shoranpur";
    public static final String Mangalore = "Mangalore";
    public static final String Madgaon = "Madgaon";
    public static final String Pune = "Pune";
//    public static final String Itarsi = "Itarsi";
    public static final String Patna = "Patna";
    public static final String New_Jalpaiguri = "New Jalpaiguri";
    public static final String Guwahati = "Guwahati";
    public static final String CHN = "CHN";
    public static final String SLM = "SLM";
    public static final String BLR = "BLR";
    public static final String KRN = "KRN";
    public static final String NGP = "NGP";
    public static final String ITJ = "ITJ";
    public static final String BPL = "BPL";
    public static final String AGA = "AGA";
    public static final String NDL = "NDL";
    public static final String TVC = "TVC";
    public static final String SRR = "SRR";

    public static final String MAQ = "MAQ";
    public static final String MAO = "MAO";
    public static final String PNE = "PNE";
    public static final String PTA = "PTA";
    public static final String NJP = "NJP";
    public static final String GHY = "GHY";
    public static final int dis1 = 1200;
    public static final int dis2 = 0;
    public static final int dis3 = 350;
    public static final int dis4 = 550;
    public static final int dis5 = 900;
    public static final int dis6 = 1200;
    public static final int dis7 = 1600;
    public static final int dis8 = 1900;
    public static final int dis9 = 2000;
    public static final int dis10 = 2500;
    public static final int dis11 = 2700;
    public static final int dis12 = 0;
    public static final int dis13 = 300;
    public static final int dis14 = 600;
    public static final int dis15 = 1000;
    public static final int dis16 = 1400;
    public static final int dis17 = 2000;
    public static final int dis18 = 2400;
    public static final int dis19 = 2700;
    public static final int dis20 = 2800;
    public static final int dis21 = 3800;
    public static final int dis22 = 4200;
    public static final int dis23 = 4700;
    public static final int dis24 = 2000;
    public static final int PRIME = 31;
    public static final int TEMP = 1;
    public static final int HyderabadDistance = 0;
    public static final int FIRST = 1;
    public static final int SECOND = 2;







}
